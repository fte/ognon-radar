...
# Committed OpenAPI YAML document (generated by `make openapi`, verified by CI).
# FastAPI only serves /openapi.json natively; this route makes the versioned
# YAML artifact reachable in prod, at both the conventional `.yml` and `.yaml`
# extensions. `include_in_schema=False` keeps these routes out of the spec
# document itself, so the committed openapi.json/openapi.yaml never drift.
...
async def openapi_yaml(request: Request):
    """
    Serve the versioned OpenAPI contract as YAML.

    Reads the committed `openapi.yaml` (kept in sync with the app by
    `make openapi` / `make openapi-check`). Both `/openapi.yml` and
    `/openapi.yaml` are served, as consumers conventionally use either
    extension.
    """
    if not OPENAPI_YAML_PATH.exists():
        return JSONResponse(
            ...
            content={
                "detail": (
                    "openapi.yaml not available on this deployment — "
                    "regenerate it with `make openapi` and redeploy."
                )
            },
        )
    return Response(
        content=OPENAPI_YAML_PATH.read_bytes(),
